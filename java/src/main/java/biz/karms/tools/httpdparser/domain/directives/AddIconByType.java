 ...
 */
public class AddIconByType implements Node {

    private final String value;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/mod_autoindex.html#addiconbytype";

    public AddIconByType(String value, Node parent) {
        if (StringUtils.isBlank(value)) {
            throw new IllegalArgumentException("AddIconByType must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost || parent instanceof Directory)) {
            throw new IllegalArgumentException("AddIconByType seems to be buried in an illegal context. See " + doc);
        }
        this.value = value;
        this.parent = parent;
    }
