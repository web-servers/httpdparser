 ...
 */
public class AddLanguage implements Node {

    private final String value;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/mod_mime.html#addlanguage";

    public AddLanguage(String value, Node parent) {
        if (StringUtils.isBlank(value)) {
            throw new IllegalArgumentException("AddLanguage must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost || parent instanceof Directory)) {
            throw new IllegalArgumentException("AddLanguage seems to be buried in an illegal context. See " + doc);
        }
        this.value = value;
        this.parent = parent;
    }
