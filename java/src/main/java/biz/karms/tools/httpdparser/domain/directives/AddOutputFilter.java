 ...
 */
public class AddOutputFilter implements Node {

    private final String filter;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/mod_mime.html#addoutputfilter";

    public AddOutputFilter(String filter, Node parent) {
        if (StringUtils.isBlank(filter)) {
            throw new IllegalArgumentException("AddOutputFilter must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost || parent instanceof Directory)) {
            throw new IllegalArgumentException("AddOutputFilter seems to be buried in an illegal context. See " + doc);
        }
        this.filter = filter;
        this.parent = parent;
    }
