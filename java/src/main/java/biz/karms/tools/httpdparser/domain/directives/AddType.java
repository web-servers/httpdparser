 ...
 */
public class AddType implements Node {

    private final String mediaTypeExtension;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/mod_mime.html#addtype";

    public AddType(String mediaTypeExtension, Node parent) {
        if (StringUtils.isBlank(mediaTypeExtension)) {
            throw new IllegalArgumentException("AddType must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost || parent instanceof Directory)) {
            throw new IllegalArgumentException("AddType seems to be buried in an illegal context. See " + doc);
        }
    ...
    public String toString() {
        return "AddType " + mediaTypeExtension;
    }
}
