...
public class Alias implements Node {
    ...
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/mod_alias.html#alias";

    public Alias(String path, Node parent) {
        if (StringUtils.isBlank(path)) {
            throw new IllegalArgumentException("Alias must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost|| parent instanceof Directory)) {
            throw new IllegalArgumentException("Alias seems to be buried in an illegal context. See " + doc);
        }
        this.path = path;
        this.parent = parent;
    }
