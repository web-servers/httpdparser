 ...
 */
public class DAV implements Node {

    private final String value;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/mod_dav.html#dav";

    public DAV(String value, Node parent) {
        if (StringUtils.isBlank(value)) {
            throw new IllegalArgumentException("DAV must not be empty. See " + doc);
        }
        if (!(parent instanceof Directory)) {
            throw new IllegalArgumentException("DAV seems to be buried in an illegal context. See " + doc);
        }
        this.value = value;
        this.parent = parent;
    }
