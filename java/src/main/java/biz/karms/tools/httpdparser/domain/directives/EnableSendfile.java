 ...
 */
public class EnableSendfile implements Node {

    private final String onOff;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/core.html#enablesendfile";

    public EnableSendfile(String onOff, Node parent) {
        if (StringUtils.isBlank(onOff)) {
            throw new IllegalArgumentException("EnableSendfile must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost|| parent instanceof Directory)) {
            throw new IllegalArgumentException("EnableSendfile seems to be buried in an illegal context. See " + doc);
        }
        this.onOff = onOff;
        this.parent = parent;
    }
