 ...
 */
public class ErrorLog implements Node {

    private final String path;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/core.html#errorlog";

    public ErrorLog(String path, Node parent) {
        if (StringUtils.isBlank(path)) {
            throw new IllegalArgumentException("ErrorLog must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost)) {
            throw new IllegalArgumentException("ErrorLog seems to be buried in an illegal context. See " + doc);
        }
        this.path = path;
        this.parent = parent;
    }
