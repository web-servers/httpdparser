 ...
 */
public class IndexOptions implements Node {

    private final String options;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/mod_autoindex.html#indexoptions";

    public IndexOptions(String options, Node parent) {
        if (StringUtils.isBlank(options)) {
            throw new IllegalArgumentException("IndexOptions must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost|| parent instanceof Directory)) {
            throw new IllegalArgumentException("IndexOptions seems to be buried in an illegal context. See " + doc);
        }
