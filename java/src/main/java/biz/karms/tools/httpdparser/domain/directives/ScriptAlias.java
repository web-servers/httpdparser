 ...
 */
public class ScriptAlias implements Node {

    private final String paths;
    private final Node parent;
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/mod_alias.html#scriptalias";

    public ScriptAlias(String paths, Node parent) {
        if (StringUtils.isBlank(paths)) {
            throw new IllegalArgumentException("ScriptAlias must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost || parent instanceof Directory)) {
            throw new IllegalArgumentException("ScriptAlias seems to be buried in an illegal context. See " + doc);
        }
        this.paths = paths;
        this.parent = parent;
    }
