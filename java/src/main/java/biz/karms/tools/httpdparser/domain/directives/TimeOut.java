...
public class TimeOut implements Node {
    ...
    public static final String doc = "https://httpd.apache.org/docs/trunk/mod/core.html#timeout";

    public TimeOut(String value, Node parent) {
        if (StringUtils.isBlank(value)) {
            throw new IllegalArgumentException("TimeOut must not be empty. See " + doc);
        }
        if (!(parent instanceof ServerConfig || parent instanceof VirtualHost)) {
            throw new IllegalArgumentException("TimeOut seems to be buried in an illegal context. See " + doc);
        }
        this.value = value;
        this.parent = parent;
    }
